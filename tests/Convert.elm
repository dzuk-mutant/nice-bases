...
testConversions label base tests =
    let
        fromIntExpec = \i s -> Expect.equal s (Base.fromInt base i)
        toIntExpec = \i s -> Expect.equal (Ok i) (Base.toInt base s)
    in
    describe label
        [ describe "fromInt" <|
...
singleConversionTest expectation index inputs =
    let
        int = Tuple.first inputs
        baseStr = Tuple.second inputs
    in
    test
